...
- What you had the coding agent do, and where you overrode it:
  I used coding agents to investigate failures, challenge the contract, implement each roadmap stage, and review new commits. I kept control of scope by preserving the evidence checkpoint and prioritizing material findings. The biggest place I overrode them was helping establish that `c8` involved more than one bug.
...
I fixed the main problems at their source: duplicate requests, overlapping workers, unsafe provider writes, and one failed run blocking recovery. I also prevent known name changes by rejecting them before writing; unexpected provider changes and later drift are detected and clearly reported instead of being shown as successful.
